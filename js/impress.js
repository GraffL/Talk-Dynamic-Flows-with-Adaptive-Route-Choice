...
( function( document, window ) {
    ...
    var showSubstepIfAny = function( step ) {
        ...
        if ( substeps.length > 0 ) {
			if( visible.length > 0) {
				step.classList.remove("on-substep-" + visible.length)
			}
			step.classList.add("on-substep-" + (visible.length+1))
    ...
    var showSubstep = function( substeps, visible ) {
		if (visible.length > 0) {
			substeps[ visible.length -1].classList.add( "substep-past" );
		}
    ...
    document.addEventListener( "impress:stepenter", function( event ) {
		...
		for ( var i = 0; i < substeps.length; i++ ) {
            step.classList.remove("on-substep-" + (i+1));
        }
    }, false );
...
( function( document, window ) {
    ...
    var tempstep = function( event ) {
		...
		var setTempClasses = function(tempstep, tempStepCounter) {
			...
			if(tempClassList) {
				tempstep.className = tempstep.className.replace(new RegExp('tempClass-[A-Za-z0-9]*', 'g'), '');
